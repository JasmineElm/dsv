...
![creating a virtual environment](Assets/10.1/image.png) This was the case for
me, I wanted to explicitly use Python3.12 I have named my virtual environment
`.venv`. This is a common name, and if you are using a Linux, or MacOS machine,
the directory will be hidden by default, which will keep your working directory
clean. You are free to name your virtual environment whatever you like, but it
is good practice give it a short name without spaces.
...
we can doe this using pip's `list` command. This will list all the libraries
that are installed in the virtual environment. Run the following command:
...
![Requirements.txt has been created](Assets/10.1/image-6.png) If we open this
file, we will see a list of all the libraries that are installed in our virtual
...
named my notebook "My_First_Notebook". ![alt text](Assets/10.1/image-11.png)
